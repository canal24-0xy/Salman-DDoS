...
def status_print(ip,port,thread_id,rps,path_get):
    print(f"{Fore.LIGHTWHITE_EX}f A D H I L {Fore.CYAN}▒ {Fore.YELLOW}HTTP{Fore.CYAN} ▒{Fore.BLUE}=⟩ {Fore.LIGHTMAGENTA_EX}TARGET{Fore.GREEN}={ip}:{port} {Fore.LIGHTBLUE_EX}PATH{Fore.WHITE}={path_get} {Fore.CYAN}RPS{Fore.WHITE}={rps} {Fore.LIGHTCYAN_EX}ID{Fore.WHITE}={thread_id}{Fore.RESET}")
...
data_type_loader_packet = input(f"{Fore.YELLOW}::TYPE PACKET ==⟩" ).upper()
target_loader = input(f"{Fore.LIGHTYELLOW_EX}::IP/URL ==⟩" )
port_loader = int(input(f"{Fore.YELLOW}::P0RT ==⟩" ))
time_loader = time.time() + int(input(f"{Fore.LIGHTYELLOW_EX}::TIME ==⟩" ))
spam_loader = int(input(f"{Fore.LIGHTGREEN_EX}::SPAM ==⟩ " ))
create_thread = int(input(F"{Fore.LIGHTGREEN_EX}::CREATE TIME ==⟩" ))
