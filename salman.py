...
port_loader = int(input(f"{Fore.YELLOW}::P0RT ==⟩ {Fore.LIGHTYELLOW_EX}"))
time_loader = time.time() + int(input(f"{Fore.LIGHTYELLOW_EX}::TIME ==⟩" ))
spam_loader = int(input(f"{Fore.LIGHTGREEN_EX}::SPAM ==⟩ " ))
create_thread = int(input(F"{Fore.LIGHTGREEN_EX}::CREATE TIME ==⟩ {Fore.LIGHTYELLOW_EX}"))
booter_sent = int(input(F"{Fore.GREEN}::REQUEST SENT ==⟩ {Fore.LIGHTYELLOW_EX} "))
